...
vtkAlgorithmOutput* getImageAsAlgorithm(vtkImageData* pImage)
{
	...
	auto pInfo = pImage->GetInformation();
...
//   reader->GetOutput()->GetDimensions(imageDims);
  ...
  vtkAlgorithmOutput* pAlgOut = getImageAsAlgorithm(pImage);

  //pAlgOut->Print(cout);


  auto pExport = vtkImageExport::New();
  pExport->SetInputConnection(pAlgOut);
  //pExport->SetInputData(pImage);
  ...
  pExport->GetDataExtent(extent);

  int imageDims[3];
  pImage->GetDimensions(imageDims);


  auto nSize = pExport->GetDataMemorySize();

  // Create the c-style image to convert the VTK image to
  unsigned short* cImage = new unsigned short[imageDims[0] * imageDims[1] * imageDims[2]];

  pExport->Export(cImage);
  ...
  for (int i = 0; i < 3; i++)
  {
	...
	riw[i]->SetInputConnection(pAlgOut);
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    planeWidget[i]->SetInputConnection(pAlgOut);
